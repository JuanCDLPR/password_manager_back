...
const { generarJWT } = require("../auth/jwt");
...
const InvitationModel = require("../invitations/invitation.model");
const {
  hashInvitationToken,
...
const registrar = async (req, res) => {
  ...
  await mongoose.connection.transaction(async (session) => {
    ...
    const consumed = await InvitationModel.updateOne(
      {
        _id: invitation.id,
        status: "pending",
        expiresAt: { $gt: now },
...
const autentificarte = async (req, res) => {
  ...
  const usuario = await UserModel.findOne(query).select(
...
const refrescarToken = async (req, res) => {
  ...
  const usuario = await UserModel.findById(req.uid).select(
    "+password +tokenVersion"
  );
